...
1. 索引页（`UiIndexContext`）：没必要每行都构建上下文，只读页面无需响应式双向绑定，一个上下文即可。选择器页面类似索引页，除了默认关掉统计功能。
2. 编辑页（`UiEditorContext`）：子表如果在表格中直接编辑（CellEditor），需响应式双向绑定，整个页面需构建一个上下文树，每行一个上下文，并可通过root context访问页面上下文
