...
describe('assembleMenuItems (core)', () => {
  it('keeps parent groups when children are readable', () => {
  ...
  it('sets allowCreate on leaf features with CREATE auth', () => {
  ...
  it('hasSystemModules detects codes without a dot', () => {
    expect(hasSystemModules(sampleModules)).toBe(false)
    expect(hasSystemModules([{ moduleCode: 'B' } as any])).toBe(true)
  })
})
