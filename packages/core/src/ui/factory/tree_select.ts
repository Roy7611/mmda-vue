...
import { setTreeChildren, treeIdOf, treeShouldLoadChildren } from './tree'
...
import { treeDataProvider } from '../tree_data'
...
export type TreeSelectFieldContext = UiFieldBindContext & {
...
export function isTreeSelectReference(reference?: MetaUiFieldRef): boolean {
...
function fieldTreeFields(field: MetaUiField): UiTreeFields {
  const reference = field.reference
  const refFlds = reference?.refFlds ?? []
  const parentId = treeSelectParentFieldOf(reference)
  return {
    id: refFlds[0] ?? 'id',
...
function resolveTreeWriteback(
  ...
  if (Array.isArray(value)) {
    return value.map((id) => find(id) ?? id)
  }
...
export function treeSelectPropsFromField(
  ...
  const treeShape = treeRef ? 'TREE' : undefined
  const shapeKey = treeRef && typeof parentField === 'string' ? parentField : undefined
  const data = hasData
    ? treeSelectNodesOf({
        data: sourceRows,
        fields,
  ...
  return {
    ...
    onExpand:
      lazy && typeof context.logic?.getChildren === 'function'
        ...
        : undefined,
    onChange: (value) => {
      context.setFieldValue(field, resolveTreeWriteback(field, value, fields))
    },
    htmlAttributes: {
