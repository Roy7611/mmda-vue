...
export interface UiDiagramSelectEventArgs extends UiEventArgs {
  id: string | number | null
  kind: 'node' | 'connector' | null
}

export type UiDiagramAsideRenderer<TNode = any> = (
...
export interface UiDiagramProps<TNode = any> extends UiProps {
  ...
  renderAside?: UiDiagramAsideRenderer<TNode>
  onSelect?: (event: UiDiagramSelectEventArgs) => void
  /** 节点集变化（值变化）。Vue 的 `v-model:nodes` 糖归 vui，不进本契约。 */
  onNodesChange?: (nodes: UiDiagramNode[]) => void
  /** 连线集变化（值变化）。 */
  onConnectorsChange?: (connectors: UiDiagramConnector[]) => void
}
...
export function unimplementedDiagramPlugin<
  TNode = any,
>(): UiDiagramPlugin<TNode> {
