...
附件、报表模板的形状在 `models/file.ts`。不要从 `ApiClient` 找 `uploadAttachment` / `uploadTemplate`。会话层（vui `VuiContext`）用 `doAction` / `postBlob`。
...
| `Attachment` 专用 API | vui `VuiContext` 的 `doAction` / `postBlob` |
