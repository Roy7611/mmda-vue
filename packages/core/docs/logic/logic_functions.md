...
不含渲染器；渲染器契约在 `ui/field_factory.ts`（`UiFieldRenderer` / `UiFieldCellRenderer` / `UiCellRenderer`）与 `ui/group_factory.ts`（`UiGroupRenderer`），不在本文件。metaui/models 不要 import 本文件。
