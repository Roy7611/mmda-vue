...
// Syncfusion locale/cldr packs are direct dependencies; keep JSON imports explicit
// so bundlers can resolve them even though they are not TypeScript modules.
// @ts-ignore - @syncfusion/ej2-locale ships JSON packs without .d.ts
import enUS from "@syncfusion/ej2-locale/src/en-US.json";
// @ts-ignore - @syncfusion/ej2-locale ships JSON packs without .d.ts
import zhLocale from "@syncfusion/ej2-locale/src/zh.json";
// @ts-ignore - @syncfusion/ej2-cldr-data ships JSON packs without .d.ts
...
const zhHansLocale: Record<string, Record<string, string>> = {
  ...
  calendar: { today: "今天" },
  datepicker: { today: "今天", placeholder: "选择日期" },
  daterangepicker: {
    placeholder: "选择日期范围",
    startLabel: "开始日期",
    endLabel: "结束日期",
  ...
  datetimepicker: { today: "今天", placeholder: "选择日期时间" },
  timepicker: { placeholder: "选择时间" },
  dialog: { close: "关闭", ok: "确定", cancel: "取消" },
  dropdowns: {
...
/** React/MMDA locale → EJ2 culture name. */
...
/** Installs Syncfusion locale for a React app. Defaults to Simplified Chinese. */
export function installSyncfusionLocale(
  locale?: string | Record<string, unknown>,
): void {
  applySyncfusionLocale(locale ?? "zh");
}
