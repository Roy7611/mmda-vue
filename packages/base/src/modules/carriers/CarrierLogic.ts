...
import { type CarrierNProd, defineCarrierNProd } from '../../models/CarrierNProd';
...
export class CarrierLogic extends UiLogic<Carrier> {
	constructor(init: UiLogicInit) {
		...
		this.addRelativeLogic<CarrierCatalog>('catalogs', (master) => new CarrierCatalogLogic(this, master));
		this.addRelativeLogic<CarrierProduct>('products', (master) => new CarrierProductLogic(this, master));

		this.beforeUpload = (context: UiContext, model: Carrier, field: MetaUiField) => {
			try {
				// 上传文件编码校验补齐
				if (!model.carrierCode && (field.fieldName == 'apiProdUrl' || field.fieldName == 'apiTestUrl')) {
	...
	beforeIndex(): UiLogicFnResult<Carrier> {
		...
		if (fields.length === 0) {
			fields.push(
				this.field('carrierName').searchable(true),
			)
		}
	...
	beforeEdit() {
		...
		if (fields.length == 0) {
			fields.push(
				this.field('apiProdUrl').onChange((context, model, newVal, oldVal) => {
					if (isRefNone(model.carrierCode)) return context.uiBuilder.toast(context, {
						severity: 'error',
						summary: context.t('dialog.title.error'),
						detail: '请先填写承运商编码',
						// group: 'br',
		...
		if (groups.length == 0) {
			groups.push(
				this.group<CarrierCatalog>('catalogs')
					.addCustomAction({
				...
				this.group<CarrierProduct>('products')
					.addCustomAction({
						name: 'createContractItem',
						label: '创建',
						icon: 'far fa-plus-circle',
 ...
 * @returns 
 */
export const CarrierLogicCtor = (metaUiService: MetaUiService, router: Router, module?: Module) => new CarrierLogic({
