...
import { MetaUiService, Module, MetaUiField, UiContext, defaultPager, MetaModel, EntityState, ApiClient, ModuleOp, ModuleAuth, auth, SearchOp, hasBit } from '@mmda/core';
...
import { type UnitConversion, defineUnitConversion } from '../../models/UnitConversion';
/**
 * 计量单位交互逻辑
 * @author mmda codebot
...
export class UnitLogic extends UiLogic<Unit> {
	constructor(init: UiLogicInit) {
		...
		this.addRelativeLogic<UnitConversion>('conversions', (master) => new UnitConversionLogic(this, master));
	}
	beforeIndex(): UiLogicFnResult<Unit> {
		const { fields, groups, customActions } = super.beforeIndex();
		if (fields.length === 0) {
			fields.push(
				this.field('unitType').searchable(true),
			)
		}
	...
	beforeEdit() {
		...
		if (groups.length == 0) {
			groups.push(
				this.group<UnitConversion>('conversions')
					.addCustomAction({
			...
			propsMapper: {
				unitRight: m => {
					...
					if (Array.isArray(m.conversions) && m.conversions.length > 0) {
						const num = Number(m.conversions[m.conversions.length - 1].rowNum) + 1
						return `${num}`
					} else {
						return `${i + 1}`
					}
 ...
 * @returns 
 */
export const UnitLogicCtor = (metaUiService: MetaUiService, router: Router, module?: Module) => new UnitLogic({
