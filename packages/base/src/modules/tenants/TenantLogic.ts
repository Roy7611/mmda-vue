...
import { type UiLogicInit, UiLogic, UiGroupLogic, type UiLogicFnResult, UiViewOne, cleanProps, fasIcon } from '@mmda/vui';
import { type Tenant, defineTenant } from '../../models/Tenant';
import { type TenantModule, defineTenantModule } from '../../models/TenantModule';
...
export class TenantLogic extends UiLogic<Tenant> {
	constructor(init: UiLogicInit) {
		...
		this.addRelativeLogic<TenantModule>('modules', (master) => new TenantModuleLogic(this, master));
		this.beforeSave = (context: UiContext, model: Tenant, action: EntityAction) => {
			const { mobile } = model
		...
		if (fields.length === 0) {
			fields.push(
				...
				this.field('countryCode').setCustomCellRenderer((fld, ctx, props) => {
					...
					const namedRoute = { name: 'Country', params: { id: `${ctx.model.country.localeCode},${ctx.model.countryCode}` } };
					...
					if (!url || !refModule?.authority?.allowRead) return ui.factory.textSpan(ctx.model.customProperties.$countryCode);
					return h('div', { class: 'flex_item_center', role: `${UI_NAME}-external-link`, id: fld.fieldName, ...props }, [
						fasIcon('external-link', mergeProps(customProps, cleanProps(['class'], props ?? {}))),
	...
	beforeEdit() {
		...
		if (groups.length == 0) {
			groups.push(
				this.group<TenantModule>('modules')
					.addCustomAction({
		...
		if (fields.length === 0) {
			fields.push(
				this.field('countryCode').setCustomRenderer((fld, ctx, props) => {
					...
					if (!url || !refModule?.authority?.allowRead) return ui.factory.textSpan(ctx.model.country.briefName);
					return h('div', { class: 'flex_item_center', role: `${UI_NAME}-external-link`, id: fld.fieldName, ...props }, [
						fasIcon('external-link', mergeProps(customProps, cleanProps(['class'], props ?? {}))),
 ...
 * @returns 
 */
export const TenantLogicCtor = (metaUiService: MetaUiService, router: Router, module?: Module) => new TenantLogic({
