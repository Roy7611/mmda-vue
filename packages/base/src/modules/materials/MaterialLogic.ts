...
import { type MaterialFeature, defineMaterialFeature } from '../../models/MaterialFeature';
import { type MaterialMedia, defineMaterialMedia } from '../../models/MaterialMedia';
import { type Sku, defineSku } from '../../models/Sku';
import { type MaterialPartner, defineMaterialPartner } from '../../models/MaterialPartner';
...
export class MaterialLogic extends UiLogic<Material> {
	constructor(init: UiLogicInit) {
		...
		this.addRelativeLogic<MaterialFeature>('features', (master) => new MaterialFeatureLogic(this, master));
		this.addRelativeLogic<MaterialMedia>('medias', (master) => new MaterialMediaLogic(this, master));
		this.addRelativeLogic<Sku>('skus', (master) => new SkuLogic(this, master));
		this.addRelativeLogic<MaterialPartner>('partNos', (master) => new MaterialPartnerLogic(this, master));
	}
	async create(param: any = {}, entityUrlParam?: EntityUrlParam): Promise<Material> {
	...
	newMaterialFeature(context: UiContext, target: Material) {
		context.createSubGroupItems({
			...
			propsMapper: {
				...
				materialID: m => ({ materialID: target.materialID, materialCode: target.materialCode })
			},
	...
	*/
	newMaterialPartner(context: UiContext, target: Material) {
		context.select<Partner>({
			...
			if (selection) {
				// 取相同的数据
				const items = selection.filter((item: any) => MetaModel.hasAnyLike(target.partNos, { partnerID: item.partnerID }));
				...
				context.addSubGroupItems<MaterialPartner>({
					...
					source: selection,
					propsMapper: {
						partnerID: m => ({ partnerID: m.partnerID, partnerCode: m.partnerCode, partnerName: m.partnerName })
					},
				});
			}
		})
	...
	beforeDetails(): UiLogicFnResult<Material> {
				...
				this.field('materialPic').setCustomRenderer((fld, ctx, props) => {
					...
					return h('div', { style: { display: 'flex', justifyContent: 'flex-start', alignItems: 'center', width: '100%', height: '100%' } }, [
						ctx.uiBuilder.factory.image(fldVal, {
							width: '70',
							height: '70',
							imageStyle: { width: '70px', height: '70px', objectFit: 'contain' },
		...
		if (groups.length == 0) {
			groups.push(
				this.group<MaterialPartner>('partNos')
					.hideIf(m => !m.partNos || m.partNos.length === 0),
			);
		}
		return { fields, groups, customActions };
	}
 ...
 * @returns
 */
export const MaterialLogicCtor = (metaUiService: MetaUiService, router: Router, module?: Module) => new MaterialLogic({
 ...
 * 特征交互逻辑
 */
export class MaterialFeatureLogic extends UiGroupLogic<MaterialFeature, Material> {
 ...
 * 特征交互逻辑
 */
export class SkuMaterialFeatureLogic extends UiGroupLogic<MaterialFeature, Sku> {
...
export class SkuLogic extends UiGroupLogic<Sku, Material> {
	constructor(parent: MaterialLogic, master: Material) {
		...
		this.addRelativeLogic<MaterialFeature>('features', (master) => new SkuMaterialFeatureLogic(this, master));
		this.addRelativeLogic<MaterialMedia>('medias', (master) => new SkuMaterialMediaLogic(this, master));
	}
}
/**
 * 供货号交互逻辑
 */
export class MaterialPartnerLogic extends UiGroupLogic<MaterialPartner, Material> {
	constructor(parent: MaterialLogic, master: Material) {
	...
	beforeEdit(): UiLogicFnResult<MaterialPartner> {
		...
		if (fields.length === 0) {
			fields.push(
				this.field('packID').setSearchParam((context, model, fld) => ({ status: UsageStatus.USED }))
			)
		}
