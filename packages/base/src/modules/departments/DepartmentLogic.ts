...
export class DepartmentLogic extends UiLogic<Department> {
	constructor(init: UiLogicInit) {
		...
		this.beforeSave = (context: UiContext, model: Department, action: EntityAction) => {
			const { tel } = model
			const { $t: t } = context.globalProps

			// 检查是否为无效的单一数字（如"0"）
			if (tel && !isRefNone(tel) && tel.trim().length === 1 && /^\d$/.test(tel.trim())) {
				return Promise.reject(Error('电话号码不能是单一数字'));
			}
	...
	beforeEdit() {
		...
		if (fields.length == 0) {
			fields.push(
				...
				this.field('tel').onValidate<string>((value) => {
					if (!isNullOrUndefined(value) && value.trim().length === 1 && /^\d$/.test(value.trim())) {
						return '电话号码格式不正确';
					}
 ...
 * @returns 
 */
export const DepartmentLogicCtor = (metaUiService: MetaUiService, router: Router, module?: Module) => new DepartmentLogic({
