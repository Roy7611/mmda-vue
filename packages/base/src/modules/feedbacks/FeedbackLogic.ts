...
import { type FeedbackPhoto, defineFeedbackPhoto } from '../../models/FeedbackPhoto';
...
export class FeedbackLogic extends UiLogic<Feedback> {
	constructor(init: UiLogicInit) {
		...
		this.addRelativeLogic<FeedbackPhoto>('photos', (master) => new FeedbackPhotoLogic(this, master));
	...
	beforeEdit() {
		...
		if (groups.length == 0) {
			groups.push(
				this.group<FeedbackPhoto>('photos')
					.addCustomAction({
	...
	newFeedbackPhoto(context: UiContext, target: Feedback) {
		context.newSubGroupItem<FeedbackPhoto>({
			group: 'photos',
			sequenceKey: 'itemID',
			target,
		}).then(item => {
 ...
 * @returns 
 */
export const FeedbackLogicCtor = (metaUiService: MetaUiService, router: Router, module?: Module) => new FeedbackLogic({
