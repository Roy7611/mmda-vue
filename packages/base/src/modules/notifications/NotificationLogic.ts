...
export class NotificationLogic extends UiLogic<Notification> {
  constructor(init: UiLogicInit) {
    ...
    this.selectableList = {
      ...
      readAll: (model) =>
        model.status === NotificationStatus.SENT ||
  ...
  beforeIndex() {
    ...
    if (fields.length == 0) {
      fields.push(
        ...
        this.field("emergency").setCustomRenderer(
          (fld, ctx: UiContext<Notification>) => {
            const { model } = ctx;
        ...
        this.field("importance").setCustomRenderer(
          (fld, ctx: UiContext<Notification>) => {
            const { model } = ctx;
            const { factory } = ctx.uiBuilder;
  ...
  async getAll(params: any) {
    // todo: 1,
    params.queryParams = Object.assign(
      { sort: `noticeTime ${SortOrder.DESC}` },
      params.queryParams,
    );
  ...
  beforeSearch() {
    ...
    if (customSearchFields.length == 0) {
      customSearchFields.push(
        {
          searchLabel: "notification.searchStatus",
          searchParam: "status",
        ...
        {
          searchLabel: "notification.searchUrgency",
        ...
        {
          searchLabel: "notification.searchImportance",
