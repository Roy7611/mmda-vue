 ...
 * 工作台首页（框架无关）。
 *
 * base 与 mes 的首页是**同一个页面**（模块总数 / 待办 / 模块导航卡片），只有主标题的文案键不同，
 * 所以实现放这里一份，两个包各写一个薄包装传自己的 `titleKey` —— 免得 250 行各维护一遍。
 *
 * 只吃 core 的 `UiViewDeps`：状态走 `app`、文案走 `app.translate`、控件走 `app.ui.factory`、
 * 跳转走 `router.resolve` + `factory.link`（宿主代管这些锚点的点击），壳节点走 `render`。
 */
import type { Module, UiViewDeps } from '@mmda/core'

export interface WorkbenchOptions {
  /** 主标题文案键，例如 base 的 `home.workbench`、mes 的 `view.scheduleWorkspace`。 */
  titleKey: string
}
...
export function workbenchView<TNode>(
