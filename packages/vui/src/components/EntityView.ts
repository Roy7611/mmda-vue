...
import { hostedEntityView } from "./hosted_view";
...
export interface EntityViewOptions {
   ...
   * 新代码用 {@link resolveEntityView}，业务包不必碰框架。
   */
  resolveCustomView?: (
    repository: string,
    service: string,
  ...
  /**
   * 实体屏自定义页（**框架无关**）：业务包给 `UiEntityViewFn`，这里包成 Vue 组件。
   * 命中它就不看 `resolveCustomView`。
   */
  resolveEntityView?: (
    repository: string,
    service: string,
  ) => UiEntityViewFn<VNode> | undefined;
}
...
function renderEntityPage(
  ...
  const EntityView = options.resolveEntityView?.(repository, service);
  if (EntityView && context.many) {
    // 框架无关的自定义页：依赖（含该屏会话）在适配层里装配。
    return h(hostedEntityView(EntityView), { ctx: context });
  }
