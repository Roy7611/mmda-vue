...
1. **壳先上。** `get` 后就挂 `VuiContext`（model 可以只有 `id`），不要等第一次 `init()` 才 `buildView`。Create 一般不用 Skeleton。
...
4. **首次结束要置 `initialized`。** `VuiContext.load()` 已写 `initializedState`；EntityView 走 mixin `refresh()` 时，第一次 load 成功后同样置位，否则 Skeleton 不会收。
