...
- chrome：`sidebar`、`drawer`（drawer = Sidebar `type: Over`）、`splitter`、`toolbar`（原生命令条）、`searchRelative`（字段选择 chrome，不是 Dialog）。页头是 `buildIndexTopbar` / `buildDetailsTopbar` / `buildEditTopbar`，见 [Topbar](./topbar.md)。
