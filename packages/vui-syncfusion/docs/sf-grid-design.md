...
三层：页（`VuiContext`）→ 整表（index 即页；子表 `subGroupContext`）→ 行（`with(row)`，缓存 `@row/{id}`）。index / selector / details 渲染禁止 `with(row)`；edit 禁止列模板预建，只在回调里按需 `with`。细则见后文「调用路径与 Context」。
...
| 层 | 怎么拿 | `model` 是什么 | 干什么 |
|---|---|---|---|
| **页** | `VuiContext` | Index = 分页列表包装；Edit/Details = 主表实体 | `search`、主表保存、打开 `subGroupItem` 对话框 |
