...
describe('buildSfTreeGridColumns / sfTreeGridColumnOf', () => {
  ...
  it('reuses sfCompareColumnFilter for date/time/number when filtering is on', () => {
    ...
    // 列定义 / 过滤器工厂返回的都是联合类型，这里只关心 ui 句柄。
    const filterUi = (source: any) => (source?.filter ?? source)?.ui
    expect(filterUi(cols[0]).create).toBeTypeOf('function')
    expect(filterUi(cols[0]).read).toBeTypeOf('function')
    expect(filterUi(sfGridColumnFilterOf(day, extras)).create).toBeTypeOf('function')
    expect(filterUi(tableFilter).create).toBeTypeOf('function')
    expect(filterUi(cols[1]).create).toBeTypeOf('function')
    expect(filterUi(cols[2]).create).toBeTypeOf('function')
  })
})
