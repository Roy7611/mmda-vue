...
| `@mmda/vuix-tempis-timeline` |     | Tempis 二维时间轴画布插件，应用 `builder.use(createTempisTimelinePlugin())` + `builder.buildTempisTimeline()`（列表式时间轴仍走 `factory.timeline`） |
| `@mmda/vuix-fc-scheduler`   |      | FullCalendar 排程插件，应用 `builder.use(createFcSchedulerPlugin())` |
| `@mmda/rui`                |      | React 运行时：会话、拼屏、Builder（与 vui 对等，不互相 import） |
| `@mmda/rui-*`              | 皮肤包  | React 皮肤实现（当前 `rui-syncfusion`） |
| `@mmda/ruix-*`             |      | React 侧可选 UI 插件 |
| `@mmda/ruix-tempis-timeline` |     | Tempis 二维时间轴画布插件（React），应用 `builder.use(createTempisTimelinePlugin())` + `builder.buildTempisTimeline()` |
| `@mmda/app`                |      | 统一 SPA 壳、Router、部署入口                   |
| `@mmda/base` / `@mmda/mes` |      | 业务插件，不是独立 SPA                          |


口语「vui」指运行时；「皮肤 / skin」指 `vui-primevue` 等；「vuix」指可选 UI 插件。React 侧对等：**`rui`**（运行时）→ **`rui-*`**（皮肤）→ **`ruix-*`**（可选 UI 插件）；rui / ruix 不得从 vui / vuix 抄组件，契约一律从 `@mmda/core` 取。
...
Index 薄封装只有 `buildGantt` / `buildScheduler` / `buildKanban` / `buildDiagram` / `buildTempisTimeline`，内部都转 `plugin(name).buildUi`。Markdown、图片编辑、Ribbon、透视表、图表、AI 助手没有 Builder 具名方法，直接 `plugin('markdown-editor')?.buildUi`。`treeGrid` 是内建，不是插件。时间轴契约在 core 分两档：`ui/plugins/timeline`（列表，chrome `factory.timeline`）与 `ui/plugins/tempis_timeline`（二维画布，`builder.buildTempisTimeline`，插件 `vuix/ruix-tempis-timeline`）。Syncfusion / AgNaive 构造时自行 `use` 自带引擎；vuix 由 App `builder.use(...)` 挂上，后装覆盖皮肤默认。
...
表单标签+槽走 `factory.formField`（不要 `formItem`）。勾选 chrome 只有 `factory.checkBox`（不要两参 `checkbox`）。开关只有 `factory.switch`（不要 `toggleSwitch`）。封闭下拉走 `dropDownList`（不要 `factory.select`；`select(field)` 是选记录）。列表主路径是 `factory.table`（不要 `dataTable` / `primeVueTable` / `factory.column`）。卡片墙自己 `h` 拼，不要 `dataViewBox`。表能力开关是 `sortable` / `pageable` / `filterable` / `groupable`（缺省 table 开；子表 Builder 关）。列筛形态 `filterDisplay`: `'menu' | 'row'`（不要用 `'none'` 当总开关，用 `filterable: false`）。自定义格 `fieldCellRenderers[fieldName]`。列宽永远可拖。列筛加载 / `tableSettings` 是 Builder→皮肤 extras，不要进程序员表契约。字段配置名 `hasOneText` 落到实现 `externalLink`（`HasOneText` 别名仍有效）；文件 URL 仍是 `fileLink`。不要 `associationTable`。PascalCase 字段别名本轮仍双轨。
...
table 与 grid **实现**可落到同一皮肤表格（SF / AgGrid / Prime DataTable）。皮肤控件名仍是 `SfGrid` / `AgGrid`。`display` 仅 factory 捷径内部标签。

`scene`（`UiGridScene`）只在 **`UiGridProps`**。table 是 index 与默认 selector，不要 `scene`。`viewKind: list` 是整页形态，不是控件名。
...
| ---------- | --------------------------- |
| `index`    | 业务列表页                       |
| `selector` | 选记录弹窗里的表（**默认跟 index 一样走 table**；特殊情况才 grid + `scene: 'selector'`） |
...
| `MetaUiGroup` | 主表区或子表                       |
| `MetaUiField` | 字段声明                         |
| `MetaUiField.primaryKey` | **这个字段是不是主键**（复合时多个字段都为 true） |
| `MetaUi.primaryKey` | 主键字段名；复合为 `key1,key2` |
| 实体 `id` | `defineEntityWithId` → `defineID` 的 **getter**，不是 JSON 字段。单主键读 `row[primaryKey]`；复合客户端 `keys.map(...).join(",")` |
...
- 列表勾选：索引与选择视图（`SelectOne` / `SelectMany`）共用 `context.selectedItems` + 同一 list 表；虚拟化下按 `id` 累计，不是当前窗快照
...
| `Module.defaultFilter` | `[alias.]field[=value]` 固定字段芯片 | FilterModel JSON、`queryID;queryName` |
